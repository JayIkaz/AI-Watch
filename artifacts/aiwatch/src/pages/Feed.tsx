...
export default function Feed() {
  ...
  const statCards = [
    ...
    { label: "Pricing changes",      value: statsPricing?.total   ?? "—", icon: DollarSign,   color: "text-teal" },
    { label: "Model releases",        value: statsModels?.total    ?? "—", icon: Cpu,          color: "text-sky" },
    { label: "Vendors tracked",       value: vendorsData?.vendors?.length ?? "—", icon: Building2, color: "text-amber" },
  ];
            ...
            <div className="space-y-5">
              ...
              <div className="pt-8 flex items-center justify-between border-t border-border">
                ...
                <button
                  ...
                  disabled={!updatesData?.updates || updatesData.updates.length < limit}
                  className="px-4 py-2 rounded-xl text-sm font-medium bg-secondary text-foreground hover:bg-secondary/80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Next
                </button>
