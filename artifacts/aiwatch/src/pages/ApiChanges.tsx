...
import { Code2, Database, Zap, X } from "lucide-react";
import { useState } from "react";
import { cn } from "@/lib/utils";

const CATEGORY = "api-changelog";
      ...
      <div className="max-w-3xl">
        ...
        <div className="mb-6 flex items-start gap-4">
          <div className="p-3 rounded-xl bg-amber/10 border border-amber/20 shrink-0">
            <Code2 className="w-6 h-6 text-amber" />
          </div>
        ...
        <div className="mb-6 space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {vendorLabel && (
              <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber/15 border border-amber/40 text-amber">
                {vendorLabel}
              </span>
            )}
            {hasFilters && (
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-muted-foreground bg-card/30 border border-dashed border-border rounded-3xl">
            ...
            <p className="text-lg font-medium text-foreground mb-1">
              {hasFilters ? "No API changes match these filters" : "No API changes found"}
            </p>
            <p className="text-sm text-center max-w-md">
              {hasFilters
                ? "Try clearing filters or selecting a different vendor."
                : "API changelog entries will appear here as they're detected."}
            </p>
            {hasFilters && (
              <button
                onClick={resetFilters}
