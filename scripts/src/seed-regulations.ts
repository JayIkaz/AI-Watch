import { db, regulationItemsTable, type InsertRegulationItem } from "@workspace/db";

// The six items previously hardcoded in
// artifacts/aiwatch/src/data/regulations.ts. Their original static `id`
// strings are reused as the deduplication hash so this script is safe to
// re-run without creating duplicates, and so a future ingestion pipeline
// can recognise these as already-seeded.
const regulations: (InsertRegulationItem & { deduplicationHash: string })[] = [
  {
    deduplicationHash: "eu-ai-act-2026-obligations",
    ...
    lastVerified: new Date("2026-07-18T09:00:00Z"),
    relevance: 0.96,
  ...
  {
    deduplicationHash: "eu-digital-omnibus-ai",
    ...
    lastVerified: new Date("2026-07-17T16:30:00Z"),
    relevance: 0.84,
  ...
  {
    deduplicationHash: "eu-intimate-content-prohibition",
    title: "EU prohibition on AI tools for non-consensual intimate imagery and CSAM takes effect 2 December 2026",
    summary:
    ...
    lastVerified: new Date("2026-07-16T10:00:00Z"),
    relevance: 0.88,
  ...
  {
    deduplicationHash: "colorado-ai-act-enforcement",
    title: "Colorado AI Act now in force: attorney general enforcement underway for high-risk AI systems",
    summary:
    ...
    lastVerified: new Date("2026-07-18T15:00:00Z"),
    relevance: 0.91,
  ...
  {
    deduplicationHash: "us-eo-state-preemption",
    ...
    lastVerified: new Date("2026-07-17T08:00:00Z"),
    relevance: 0.82,
  ...
  {
    deduplicationHash: "nist-ai-rmf-update",
    ...
    lastVerified: new Date("2026-07-14T12:00:00Z"),
    relevance: 0.86,
...
async function main() {
  for (const item of regulations) {
    const result = await db
      .insert(regulationItemsTable)
      .values(item)
      .onConflictDoNothing()
      .returning({ id: regulationItemsTable.id, title: regulationItemsTable.title });

    if (result.length === 0) {
      console.log(`Skipped (already seeded): ${item.title}`);
    } else {
